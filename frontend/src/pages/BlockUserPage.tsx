...
export default function BlockUserPage() {
  ...
  const handleUnBlock = (user_id: string) => {
    ...
    <div className='flex flex-1 bg-gray-200 dark:bg-gray-900 flex-col text-foreground'>
      ...
      {listBlockedUser.length > 0 ? (
        <div>
          ...
          <div className='text-foreground bg-white dark:bg-background rounded-sm mx-4 py-2'>
              ...
              {listBlockedUser.map((user, index) => {
                return (
                  ...
                  >
                    ...
                    <Button onClick={() => handleUnBlock(user._id)} className='cursor-pointer'>
                      Bỏ chặn
                    </Button>
                  </div>
                )
