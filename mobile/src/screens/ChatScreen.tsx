...
const ChatScreen = ({ route }: any) => {
  ...
  useFocusEffect(
    useCallback(() => {
      // 1. Tải Private Key mỗi khi màn hình này được focus
      ...
      // 2. Fetch data User và list hội thoại
  ...
  const fetchBlockedUsers = async () => {
    try {
      ...
      const ids = (res.data?.result || []).map((u: any) => (u._id || '').toString())
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Ionicons
                name={pinnedIds.has(selectedConvForPin?._id) ? 'pin-outline' : 'pin'}
                size={20}
                color={COLORS.accent}
              />
