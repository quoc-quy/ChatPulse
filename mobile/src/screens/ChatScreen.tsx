...
const ChatScreen = () => {
  ...
  const formatTimeZalo = (dateString: string) => {
    ...
    if (diffHours < 24 && date.getDate() === now.getDate()) return `${diffHours} giờ`;
  ...
  const getChatDetails = (item: any) => {
    ...
    return { chatName, chatAvatarUrl, isOnline };
  };
  ...
  const renderItem = ({ item }: any) => {
    ...
    if (item.type === 'group' && item.lastMessage?.sender_id) {
      if (item.lastMessage.sender_id === currentUserId) {
        ...
        const sender = item.participants?.find((p: any) => p._id === item.lastMessage.sender_id);
        if (sender) {
          const shortName = sender.userName ? sender.userName.split(' ').pop() : 'Ai đó';
      ...
      >
        ...
        <View style={styles.chatContent}>
          <View style={styles.chatHeader}>
            ...
            <Text style={[styles.time, unread > 0 && styles.unreadTime]}>{time}</Text>
          </View>
          ...
          <View style={styles.chatFooter}>
            ...
            {unread > 0 && (
              <View style={styles.badge}>
                <Text style={styles.badgeText}>{unread > 5 ? 'N' : unread}</Text>
              </View>
            )}
          </View>
        </View>
    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerIcons}>
        <TouchableOpacity style={styles.iconBtn}>
          <Ionicons name="qr-code-outline" size={24} color={COLORS.headerText} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.iconBtn}>
...
const getStyles = (COLORS: any) => StyleSheet.create({
  ...
  tabButton: { paddingVertical: 6, paddingHorizontal: 16, borderRadius: 20, marginRight: 10 },
  ...
  onlineDot: { position: 'absolute', right: 0, bottom: 2, width: 14, height: 14, borderRadius: 7, backgroundColor: COLORS.success, borderWidth: 2, borderColor: COLORS.surface },
  chatContent: { flex: 1 },
  ...
  message: { color: COLORS.textLight, fontSize: 14, flex: 1, marginRight: 12 },
  unreadMessage: { color: COLORS.text, fontWeight: "600" },
  badge: { backgroundColor: COLORS.badge, minWidth: 20, height: 20, borderRadius: 10, justifyContent: "center", alignItems: "center", paddingHorizontal: 5 },
