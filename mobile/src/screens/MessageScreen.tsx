...
import { FileSummaryModal } from '../components/message/FileSummaryModal'
...
function parseMediaContent(content: string): FilePayload[] {
  try {
    ...
    return [
      { url: content, originalName: content.split('/').pop() || 'file', size: 0, mimeType: '' }
    ]
  } catch {
    return [
      { url: content, originalName: content.split('/').pop() || 'file', size: 0, mimeType: '' }
  ...
  try {
    ...
    if (stored) {
      ...
      const index = archivedArray.findIndex(
        (key: string) => key.startsWith(`${conversationId}:`) || key === conversationId
      )
      if (index !== -1) {
...
const MessageScreen = () => {
  ...
  const [isOnline, setIsOnline] = useState<boolean>(
    route.params?.isOnline ?? partnerInit.isOnline ?? false
  )
  const [lastOnlineTime, setLastOnlineTime] = useState<string | null>(
    route.params?.lastOnline ??
      partnerInit.lastActiveAt ??
      partnerInit.lastOnline ??
      partnerInit.last_active ??
      null
  )
  const [nowTick, setNowTick] = useState(Date.now())

  const [isNotFriendState, setIsNotFriendState] = useState<boolean>(
    route.params?.isFriend === false
  )
  ...
  const [summaryData, setSummaryData] = useState<any>(null) // ConversationSummary | null
  const [showFileSummaryModal, setShowFileSummaryModal] = useState(false)
  const [fileSummaryMessageId, setFileSummaryMessageId] = useState<string | null>(null)
  ...
  const headerStatusText = useMemo(() => {
    ...
    if (diffHours < 24 && last.getDate() === now.getDate())
        ...
        try {
          ...
          if (friendInfo) {
            ...
            if (friendInfo.lastOnline || friendInfo.last_active || friendInfo.lastActiveAt) {
              setLastOnlineTime(
                friendInfo.lastActiveAt || friendInfo.lastOnline || friendInfo.last_active
              )
            }
          }
        } catch (error) {}
  ...
  useEffect(() => {
    ...
    const handlePinnedUpdate = (data: any) => {
      if (data.conversationId === route.params.id) setPinnedMessages(data.pinnedMessages)
    }
      ...
      if (conversationId && resolvedUserId) {
        ...
        try {
          ...
          if (conv) {
            ...
            let pMembers = rawMembers
              .map((m: any) => (m.userId ? m.userId : m))
              .filter((m: any) => m != null)

            const isPopulated =
              pMembers.length > 0 &&
              typeof pMembers[0] === 'object' &&
              (pMembers[0].avatar || pMembers[0].avatarUrl || pMembers[0].userName)
            ...
            if (!isGroup) {
              finalMembers = finalMembers.filter(
                (m: any) => m._id !== resolvedUserId && m.id !== resolvedUserId
              )
              if (finalMembers.length > 0) {
                if (!newAvatarUrl)
                  newAvatarUrl = finalMembers[0].avatar || finalMembers[0].avatarUrl || ''
                ...
                if (
                  finalMembers[0].lastActiveAt ||
                  finalMembers[0].lastOnline ||
                  finalMembers[0].last_active
                ) {
                  setLastOnlineTime(
                    finalMembers[0].lastActiveAt ||
                      finalMembers[0].lastOnline ||
  ...
  const handleForward = () => {
    setShowMenu(false)
    if (selectedMsg) navigation.navigate('ForwardMessageScreen', { messageId: selectedMsg._id })
  }
      ...
      setMessages((prev) => [
        {
          ...
          conversationId,
          type: 'system_error',
          content: error.response?.data?.message || t.messageAttachmentFailed || 'Lỗi',
          createdAt: new Date().toISOString()
        },
  ...
  const handleSend = async () => {
    ...
    if (textToSend.length > 2000) {
      Alert.alert(t.error || 'Cảnh báo', `Tin nhắn quá dài (${textToSend.length}/2000 ký tự).`)
      return
    }
        ...
        setMessages((prev) => [
          {
            ...
            conversationId,
            type: 'system_error',
            content: error.response?.data?.message || 'Lỗi gửi',
            createdAt: new Date().toISOString()
          },
          ...prev.filter((msg) => msg._id !== tempId)
        ])
  ...
  const handleSummarizeChat = async () => {
    ...
    try {
      ...
      setSummaryData({
        topic: 'Máy chủ AI đang bận. Vui lòng thử lại sau.',
        decisions: [],
        openQuestions: [],
        actionItems: []
      })
  ...
  // [MỚI] Tóm tắt file / ảnh — gọi POST /messages/:messageId/summarize
  const handleSummarizeFile = (messageId: string) => {
    setFileSummaryMessageId(messageId)
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg._id === selectedMsg._id ? { ...msg, type: 'revoked', content: '' } : msg
        )
      )
    } catch (error: any) {
  ...
  const renderPinnedMessageContent = (message: any) => {
    ...
    if (type === 'text')
      return message.content.startsWith('@PulseAI ')
        ? message.content.substring(9)
        : message.content
    try {
    ...
    <SafeAreaView style={styles.container}>
      ...
      {!isGroup && isNotFriendState && (
        <View
          style={{
            ...
            alignItems: 'center',
            borderBottomWidth: 1,
            borderBottomColor: isDarkMode ? '#7f1d1d' : '#FCA5A5'
          }}
        >
          <Ionicons
          ...
          />
          <Text
            style={{ color: isDarkMode ? '#FCA5A5' : '#EF4444', fontSize: 13, fontWeight: '500' }}
          >
      ...
      >
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <MessageBubble
                ...
                isMe={(item.sender?._id || item.senderId) === currentUserId}
                messages={messages}
                highlightedMsgId={highlightedMsgId}
                formatMessageDate={formatMessageDate}
                formatTime={formatTime}
                parseMediaContent={parseMediaContent}
                ...
                openReactionDetails={openReactionDetails}
        ...
        <MessageInput
          ...
          disbandMessage={disbandMessage}
          handleDeleteDisbandedChat={handleDeleteDisbandedChat}
          formatMessageDate={formatMessageDate}
          inputText={inputText}
      ...
      <PinnedModal
        ...
        renderPinnedMessageContent={renderPinnedMessageContent}
      ...
      <ReactionModal
        ...
        reactionGroupsForModal={reactionGroupsForModal}
        reactionUsersForModal={reactionUsersForModal}
        COLORS={COLORS}
        styles={styles}
        t={t}
        isDarkMode={isDarkMode}
      ...
      <FileSummaryModal
        isOpen={showFileSummaryModal}
        onClose={() => setShowFileSummaryModal(false)}
        messageId={fileSummaryMessageId}
        isDarkMode={isDarkMode}
        COLORS={COLORS}
      />
      ...
      <MessageMenuModal
        ...
        menuPos={menuPos}
        setEmojiStripWidth={setEmojiStripWidth}
        emojiPanResponder={emojiPanResponder}
        hoveredReaction={hoveredReaction}
        handleRemoveAllReactions={handleRemoveAllReactions}
...
const getStyles = (COLORS: any, isDarkMode: boolean) =>
  StyleSheet.create({
    ...
    messageTime: {
      fontSize: 11,
      color: isDarkMode ? COLORS.textLight : '#6B7280',
      marginTop: 4,
      alignSelf: 'flex-end'
    },
