...
const MessageScreen = () => {
  ...
  const handleSuggestReply = async () => {
    ...
    try {
      ...
      if (res.data?.result) {
        // Tự động chèn cờ hiệu @PulseAI vào đầu tin nhắn
  ...
  const handleSummarizeChat = async () => {
    ...
    try {
      ...
      setIsSummarizing(false); // 🔥 nhớ tắt khi lỗi
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((msg) => {
          ...
          const nextReactions = [
            ...currentReactions,
            { emoji, user_id: currentUserId, user: { _id: currentUserId, userName: "Bạn" } },
          ];
      ...
      setReactionDetailMessage((prev: any) => {
        ...
        return {
          ...prev,
          reactions: Array.isArray(apiReactions) ? apiReactions : prev.reactions,
        };
      });
    } catch (error) {
  ...
  const getReactionUserName = (reaction: any) => {
    ...
    if (reactionUserId && reactionUserId === currentUserId?.toString?.()) return "Bạn";
    return reaction?.user?.userName || reaction?.user?.displayName || reaction?.userName || "Người dùng";
  };
  ...
  const reactionUsersForModal = useMemo(() => {
    const groupByUserAndEmoji = (reactions: any[]) => {
      const grouped = new Map<string, { userId: string; userName: string; avatar: string; emoji: string; count: number; }>();
      reactions.forEach((reaction: any) => {
        const emoji = reaction?.emoji;
        if (!emoji) return;
        ...
        if (!grouped.has(key)) {
          grouped.set(key, { userId, userName: getReactionUserName(reaction), avatar: getReactionUserAvatar(reaction), emoji, count: 0 });
        }
    ...
    const groupByUserAllEmojis = (reactions: any[]) => {
      const grouped = new Map<string, { userId: string; userName: string; avatar: string; totalCount: number; emojiCounts: Record<string, number>; }>();
      reactions.forEach((reaction: any) => {
        const emoji = reaction?.emoji;
        if (!emoji) return;
        const userId = getReactionUserId(reaction) || "unknown";
        if (!grouped.has(userId)) {
          grouped.set(userId, { userId, userName: getReactionUserName(reaction), avatar: getReactionUserAvatar(reaction), totalCount: 0, emojiCounts: {} });
        }
      ...
      return Array.from(grouped.values())
        .map((row) => ({ ...row, emojis: Object.entries(row.emojiCounts).sort((a, b) => b[1] - a[1]).map(([emoji]) => emoji) }))
        .sort((a, b) => b.totalCount - a.totalCount);
    };

    if (reactionFilter !== "ALL") {
      const selectedGroup = reactionGroupsForModal.find((group) => group.emoji === reactionFilter);
      return groupByUserAndEmoji(selectedGroup?.users || []);
    }
    return groupByUserAllEmojis(reactionGroupsForModal.flatMap((group) => group.users));
  ...
  const handleSend = async () => {
    // Không gửi tin nhắn rỗng hoặc chỉ có mỗi chữ @PulseAI 
      ...
      <Text style={styles.aiText}>
        {parts.map((part, index) => {
          if (part.startsWith("[xem:") && part.endsWith("]")) {
            ...
            return (
              <Text key={index} style={styles.aiLink} onPress={() => scrollToMessage(msgId)}>
                {" "}(Xem){" "}
              </Text>
            );
          }
  ...
  const renderMessage = ({ item, index }: { item: any; index: number }) => {
      ...
      return (
        <View>
          ...
          <View style={styles.systemMessageWrapper}>
            <Text style={[styles.systemMessageText, { color: isDarkMode ? "rgba(255,255,255,0.45)" : "rgba(0,0,0,0.4)" }]}>
              {item.content}
            </Text>
          </View>
        </View>
    ...
    // TÁCH VÀ HIGHLIGHT @PulseAI Ở BONG BÓNG CHAT
          ...
          <View style={[styles.messageContent, isMe ? { alignItems: "flex-end" } : { alignItems: "flex-start" }]}>
            <TouchableOpacity onPress={() => handleDoubleTap(item)} onLongPress={(e) => handleLongPress(e, item)} activeOpacity={0.9}>
              ...
              >
                ...
                >
                  {isRevoked ? "Tin nhắn đã được thu hồi" : (
                    <>
                      {isAiGenerated && (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <LinearGradient colors={[COLORS.primary, COLORS.accent]} style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
              ...
              {isMutedState && <Ionicons name="notifications-off" size={14} color="rgba(255,255,255,0.75)" />}
            </View>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate("ConversationDetail", { id: conversationId, name: chatName, isGroup: isGroup })
            }
          >
      ...
      <KeyboardAvoidingView style={styles.chatArea} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={isFetchingMore} onRefresh={loadMoreMessages} tintColor={COLORS.primary} />}
          ListHeaderComponent={
        ...
        <View style={styles.inputContainer}>
          ...
          {/* Ô NHẬP TEXT ĐƯỢC XỬ LÝ ĐỂ HIỆN @PulseAI MÀU TÍM */}
          <View style={[styles.textInput, { flexDirection: "row", alignItems: "center", paddingHorizontal: 0 }]}>
            {inputText.startsWith("@PulseAI ") && (
              <Text style={{ color: "#C084FC", fontWeight: "900", paddingLeft: 16 }}>@PulseAI</Text>
            )}
            <TextInput
              style={{
                flex: 1,
              ...
              onKeyPress={({ nativeEvent }) => {
                // Xoá tag AI nếu backspace khi input trống
        ...
        <Pressable style={styles.overlay} onPress={() => setShowMenu(false)}>
          <View style={[styles.menuBox, { top: menuPos.y }]}>
            <View style={styles.emojiRow}>
              <View style={styles.emojiStrip} onLayout={(event) => setEmojiStripWidth(event.nativeEvent.layout.width)} {...emojiPanResponder.panHandlers}>
                {REACTION_LIST.map((e) => {
      ...
      <Modal visible={showReactionDetails} transparent animationType="fade">
        {/* ... MÃ MODAL REACTION DETAILS GIỮ NGUYÊN ... */}
            ...
            <View style={styles.reactionDetailBody}>
              ...
              <View style={styles.reactionUsersCol}>
                <FlatList
                  ...
                  ListEmptyComponent={<Text style={styles.reactionEmptyText}>Chưa có biểu cảm</Text>}
                />
              </View>
            </View>
          </Pressable>
      ...
      <Modal visible={showAiModal} transparent animationType="fade">
        {/* ... MÃ MODAL AI SUMMARY GIỮ NGUYÊN ... */}
...
const getStyles = (COLORS: any, isDarkMode: boolean) =>
  StyleSheet.create({
    ...
    bubbleOther: { backgroundColor: COLORS.surface, borderWidth: isDarkMode ? 1 : 1, borderColor: COLORS.border, borderBottomLeftRadius: 2 },
    ...
    inputContainer: { flexDirection: "row", padding: 10, backgroundColor: COLORS.surface, alignItems: "center", borderTopWidth: 1, borderColor: COLORS.border },
    attachBtn: { padding: 8 },
