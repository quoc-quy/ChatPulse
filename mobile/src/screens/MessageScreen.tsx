...
const MessageScreen = () => {
  ...
  const fetchMuteState = async (userId: string) => {
    ...
    try {
      ...
      const myMember = (conv?.members || []).find((m: any) => m.userId?.toString() === userId)
  ...
  const handleToggleReact = async (message: any, emoji: string) => {
    ...
    try {
      ...
      const apiReactions =
        response?.data?.result?.reactions || response?.data?.result?.value?.reactions || null
  ...
  const handleRemoveAllReactions = async (message: any) => {
    ...
    try {
      ...
      const apiReactions =
        response?.data?.result?.reactions || response?.data?.result?.value?.reactions || []
      setMessages((prev) =>
        prev.map((msg) => (msg._id === message._id ? { ...msg, reactions: apiReactions } : msg))
      )
      setReactionDetailMessage((prev: any) => {
        if (!prev || prev._id !== message._id) return prev
  ...
  const getReactionUserName = (reaction: any) => {
    ...
    return (
      reaction?.user?.userName || reaction?.user?.displayName || reaction?.userName || 'Người dùng'
    )
  }
  ...
  const renderMessage = ({ item, index }: { item: any; index: number }) => {
    ...
    const isSameSenderAsNext =
      nextItem &&
      (nextItem.sender?._id || nextItem.senderId) === (item.sender?._id || item.senderId)
