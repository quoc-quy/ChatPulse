...
export function LoginForm({ navigation, onLoginSuccess }: LoginFormProps) {
  ...
  const { connectSocket, disconnectSocket } = useChatContext();
  ...
  useEffect(() => {
    AsyncStorage.getItem("custom_ip").then((val) => {
      if (val) setCustomIp(val);
    });
  }, []);
          ...
          >
            <View style={[styles.card, { backgroundColor: colors.card }]}>
              ...
              {showIpInput && (
                ...
                >
                  <Input
                    label="IP Server Tùy Chọn (Developer)"
                    placeholder="Ví dụ: 192.168.1.103"
                    value={customIp}
                    onChangeText={async (text) => {
                      setCustomIp(text);
                      if (text.trim()) {
                        await AsyncStorage.setItem("custom_ip", text.trim());
                      } else {
                        await AsyncStorage.removeItem("custom_ip");
                      }
                      disconnectSocket();
                      setTimeout(() => connectSocket(), 300);
                    }}
                  />
                  <TouchableOpacity onPress={() => setShowIpInput(false)}>
                    <Text
                      style={{
              ...
              <View style={styles.separatorContainer}>
                ...
                />
                <Text
                  style={[styles.sepText, { color: colors.mutedForeground }]}
                >
                  Or continue with
                </Text>
                <View
